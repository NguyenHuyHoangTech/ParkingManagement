    ...
    <Modal
      title={
        <div className="flex items-center gap-2 text-blue-700">
          ...
          <span className="text-lg">Building Rules & Regulations</span>
        </div>
      }
      open={isOpen}
      onCancel={onClose}
        ...
        <div className="py-2">
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6">
            ...
            <div className="mt-2 text-sm text-slate-600 flex flex-col gap-1">
              <Text><strong className="text-slate-500">Address:</strong> {buildingProfile.address}</Text>
          ...
          <Title level={5} className="text-slate-800 border-b pb-2 mb-4">List of Rules & Parking Regulations</Title>
          <div className="text-slate-700 whitespace-pre-wrap leading-relaxed text-sm bg-white p-4 rounded border border-gray-100 shadow-inner min-h-[200px]">
            {buildingProfile.rules ? buildingProfile.rules : <Text type="secondary" italic>No rules configured yet.</Text>}
          </div>
        </div>
      ) : (
        <div className="py-12 text-center">
          <Text type="secondary">Failed to load building information.</Text>
        </div>
      )}
    </Modal>
  );
